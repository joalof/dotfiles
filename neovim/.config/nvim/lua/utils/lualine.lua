...
local function cmp_source(name, icon)
    local started = false
    ...
    local colors = {
        ok = highlight.get_fg_color("Special"),
        error = highlight.get_fg_color("DiagnosticError"),
        pending = highlight.get_fg_color("DiagnosticWarn"),
    }

    return {
        function()
            return icon or require("profile.icons").kinds[name:sub(1, 1):upper() .. name:sub(2)]
        end,
        cond = function()
            return status() ~= nil
        end,
    ...
    if not lualine_hl_group then
        ...
        local gui = vim.tbl_filter(function(x)
            return x
                ...
                highlight.extract_colors(hl_group, "bold") and "bold",
                highlight.extract_colors(hl_group, "italic") and "italic",
            })

        lualine_hl_group = component:create_hl({
            fg = highlight.extract_colors(hl_group, "fg"),
